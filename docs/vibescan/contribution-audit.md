...
| Comparative advantage over eslint-plugin-security on DVNA first-party code | Implemented via tool runs and manual adjudication artifacts under `benchmarks/results/legacy/` | Summarized in `benchmarks/results/legacy/dvna-evaluation.md` | Reported as preliminary: VibeScan vs eslint on first-party adjudicated TPs | **Partially supported** |
| Complete multi-baseline comparison including Bearer | Planned baseline in `benchmarks/results/legacy/dvna-evaluation.md` and `benchmarks/results/legacy/bearer-dvna.txt` | Bearer command guidance exists | Bearer run is explicitly pending in current environment | **Future work** |
...
1. **Claim:** DVNA comparison is presented with a strong numeric contrast vs eslint.  
   **Evidence status:** Supported as **preliminary/manual adjudication** in `benchmarks/results/legacy/dvna-evaluation.md`.  
