...
function extractOperationsFromDoc(doc: unknown, specFile: string): SpecOperation[] {
...
export function loadOpenApiOperations(absPath: string): SpecOperation[] {
  ...
  return extractOperationsFromDoc(doc, absPath);
}
...
export function runOpenApiDriftAudit(routes: RouteNode[], specPaths: string[]): Finding[] {
  ...
  for (const sp of specPaths) {
    const ops = loadOpenApiOperations(sp);
    for (const op of ops) {
