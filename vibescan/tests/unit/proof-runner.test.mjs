...
`,
      ...
      assert.strictEqual(log.summary.flaky, 0);
      assert.strictEqual(log.entries[0].stability, "stable");
      assert.strictEqual(log.retriesPerFile, 1);
    } finally {
      rmSync(dir, { recursive: true, force: true, maxRetries: 5, retryDelay: 50 });
    }
  });

  it("runProofHarness with retries records perRun and stable result", () => {
    const dir = mkdtempSync(join(tmpdir(), "vibescan-proof-"));
    try {
      const testFile = join(dir, "sample.test.mjs");
      writeFileSync(
...
`,
      ...
      const log = runProofHarness({ fromJson: projectJson, retries: 3 });
      assert.strictEqual(log.retriesPerFile, 3);
      assert.strictEqual(log.entries[0].perRun?.length, 3);
      assert.strictEqual(log.entries[0].stability, "stable");
    } finally {
      rmSync(dir, { recursive: true, force: true, maxRetries: 5, retryDelay: 50 });
    }
